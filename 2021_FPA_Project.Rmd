...
set.seed(42)
randtest = sample(1:35424,7085)
trans_data = t(data[,2:19])
colnames(trans_data) = paste0('t',1:ncol(trans_data))
rfdata = trans_data[,1:2]
for (col in 2:1968){
part = trans_data[, col:(col+1)]
rfdata = rbind(rfdata, part)
}
trainrf = rfdata[-randtest, ]
valrf = rfdata[randtest, ]
xtestrf = data.frame(trans_data[,1968])
xtrain_rf = data.frame(trainrf[,1])

xval_rf = data.frame(valrf[,1])

colnames(xval_rf) = colnames(xtrain_rf)
colnames(xtestrf) = colnames(xtrain_rf)

rfreg = randomForest(formula = trainrf[,2] ~., data = xtrain_rf, ntree=100)
y_pred_rf = predict(rfreg, newdata = xval_rf)

```

```{r message=FALSE, warning=FALSE}
library(Metrics)
yval_rf = as.array(data.frame(valrf[,2])[,1])
evaluation_var_rf_validation = evaluation_var(yval_rf, y_pred_rf, xval_rf)
```

In this section we adopted the random forest model with the number of trees set at 100 in machine learning. The results show that the RMSSE on the training and test subset are only 0.17 and 0.13 respectively, much lower than the VAR models in section 2 or the multiple models we adopted in the first part. So our conclusion is that the machine learning techniques with standard techniques helped.

```{r}
datatable(evaluation_var_rf_validation[,-5])
```

```{r}
ytestrf = as.array(data.frame(trans_data[,1969])[,1])
y_pred_rf_test = predict(rfreg, newdata = xtestrf)
y_ses_rf = as.numeric((y_pred_rf_test + read.csv(file='./prediction_point_test.csv')[3,])/2)
evaluation_var_rf_test = evaluation_var(ytestrf, y_pred_rf_test, xtestrf)
evaluation_var_ses_rf_test = evaluation_var(ytestrf, y_ses_rf, xtestrf)
evaluation_var_rf_test = rbind(evaluation_var_rf_test, evaluation_var_ses_rf_test) 
```

In the testing set, we also discovered that a combination of machine learning models and time series analytic technique could yield a better result. The one-point forecast has the lowest RMSE compared to other tested models except for VARs. 

```{r}
datatable(evaluation_var_rf_test[,-5])
```
